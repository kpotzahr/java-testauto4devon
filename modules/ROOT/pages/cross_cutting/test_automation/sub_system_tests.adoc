...
Usually only part of the attributes are relevant, e.g. you need a person and the relevant only relevant fact is that it is a student.
...
Keep those references at a central place in your test code, so that they can be changed easily or be configured for test run on different databases.
